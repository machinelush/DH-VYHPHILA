...
if ( ! function_exists( 'cfct_enqueue_pluggable' ) ) {
	function cfct_enqueue_pluggable() {
		if (!is_admin()) {
			wp_enqueue_script('favebusiness', get_bloginfo('template_directory') . '/assets/js/master.js', array('jquery'), CFCT_URL_VERSION);
		}
		if (is_child_theme() && !is_admin()) {
			wp_enqueue_style('favebusiness', get_bloginfo('stylesheet_url'), array(), CFCT_URL_VERSION, 'all');
		}
	}
}
